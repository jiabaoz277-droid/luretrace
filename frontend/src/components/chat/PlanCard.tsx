...
const CONCLUSION_CLS: Record<Plan["conclusion"], string> = {
  go: "bg-lime/15 text-lime",
  conditional: "bg-amber-400/15 text-amber-300",
  no_go: "bg-accent/15 text-accent",
};
...
export function PlanCard({ plan }: { plan: Plan }) {
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-xl border border-white/10 bg-white/5">
      ...
      <p className="border-t border-white/10 px-3 py-2 text-xs text-ink-soft">
      ...
      {plan.risks?.length > 0 && (
        <p className="px-3 pb-1 text-xs text-amber-300">注意：{plan.risks.join("；")}</p>
      )}
      {plan.safety?.length > 0 && (
        <p className="px-3 pb-1 text-xs font-semibold text-accent">安全：{plan.safety.join("；")}</p>
      )}
      {plan.history_note && (
        <p className="flex items-start gap-1 px-3 pb-2 text-xs text-lime">
